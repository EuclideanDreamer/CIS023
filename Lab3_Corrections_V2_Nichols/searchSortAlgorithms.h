...
#ifndef SEARCH_SORT_ALGO_H
#define SEARCH_SORT_ALGO_H


template <class elmT>
void swap(elmT list[], int first, int second) {
    
    elmT temp = list[first];
...
void recQuickSort(elmT list[], int first, int last)
{
    if (first < last)
    {
        ...
        recQuickSort(list, first, pivotIndex - 1);
        recQuickSort(list, pivotIndex + 1, last);
    }
}
